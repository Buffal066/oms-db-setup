...
SELECT * FROM SLEEK_OMS.L1_LANDING.EMPLOYEE_PER;
...
-- this query uses the results from the last query
-- if we close the session and log back in the TMP table with be gone.
...
-- per is uptp 90 days, TMP AND TRN are upto 1 day.
-- the temperary table only exists for the duration of the session regardless of the time travel duration
